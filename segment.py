...
list_scans = glob(os.path.join(config["path"]["test_scans"],"*.mhd")) #-Lists all .mhd scans.

st_scans = [os.path.basename(s) for s in list_scans]

print("st_scans", st_scans)

dataset = seg_dataset.SegmentSet(st_scans, config["path"]["test_scans"], mode="3d", scan_size=config["train3d"]["scan_size"])
...
for i in range(len(dataset)):
	segID = str(st_scans[i].replace(".mhd", ""))[-4:]
